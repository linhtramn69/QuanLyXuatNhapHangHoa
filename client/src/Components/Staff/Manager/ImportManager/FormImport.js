...
function FormImport() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const data = {
                ...
                nhanvien: user,
                noi_xuat_hang: user.noi_lam_viec.kho,
    ...
    useEffect(() => {
        const get = async () => {
            ...
            setPhieuXuat((await Service.exportService.getExport(user.noi_lam_viec.kho.ten_kho)).data);
        };
        get()
    }, []);
    useEffect(() => {
                        ...
                        <Table bordered className={cx('table-export')}>
                            ...
                            {sanphamhien.map((e, i) => (
                                <tbody>
                                    ...
                                    <td>{e.export.hsd}</td>
                                    <td>{e.ghichu}</td>
                                    <td onClick={() => (handleAddTemp(e.export))}><FontAwesomeIcon icon={faSquarePlus} /></td>
                                </tbody>
                            ...
                            <tbody>
                                {temp.map((item, i) => (
                                    <tr>
                                        <td>{item.ten_hh}</td>
                                        <td>{item.dvt}</td>
                                        <td><input style={{ width: '100%' }} type="number" max={item.sl} value={item.sl} onChange={(e) => {
                                            setTemp((prev) => {
