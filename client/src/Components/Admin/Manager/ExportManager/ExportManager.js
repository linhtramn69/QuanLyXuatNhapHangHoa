...
function ExportManager() {
    ...
    useEffect(() => {
        const fetchExports = async () => {
            setExports((await exportService.get()).data);
            setKhoTong((await khoService.getId('638f0daae08e455beca71ac2')).data);
        };
        fetchExports();
    }, []);
    return (
        <>
            <Container className={cx('')}>
                ...
                <Table bordered hover className='text-center'>
                    ...
                    <tbody>
                        {exports.map((xuat) => {
                            if (xuat.noi_xuat_hang.ten_kho === khotong.ten_kho) {
                                if (filter) {
                                            ...
                                            <tr>
                                                {(() => {
                                                    if (xuat.status === 0) {
                                                        ...
                                                        return <td><Badge bg="success">Đang chuyển hàng</Badge></td>
                                                ...
                                                <td>{xuat.noi_nhan_hang.ten_kho}</td>
